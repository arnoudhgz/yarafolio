...
4. Analyst sentiment: the supplied forecast line (consensus, target range, and when present a SB/B/H/S/SS analyst distribution + an as-of date) usually suffices; weigh a thin consensus or a stale as-of date, and search only for very fresh moves (today's up/downgrades).
