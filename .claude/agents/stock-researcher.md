...
4. **Value & Health Matrix (Buffett/Piotroski/Greenblatt)**: The strategy requires finding true value, not dying companies. Evaluate the company's competitive moat and financial trend. If the ROE, Debt/Equity, Piotroski F-Score, or Earnings Yield are missing from the prompt, run a quick WebSearch to find them. Reject the stock as a value trap (Rating C) if it is highly leveraged (Debt/Equity > 0.5), lacks a competitive edge (ROE < 15%), or has a poor Piotroski score (< 4), unless it's a specific turnaround/asset play.
...
6. Analyst sentiment: the supplied forecast line (consensus, target range, and when present a SB/B/H/S/SS analyst distribution + an as-of date) usually suffices; weigh a thin consensus or a stale as-of date, and search only for very fresh moves (today's up/downgrades).
