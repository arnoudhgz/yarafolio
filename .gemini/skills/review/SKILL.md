...
- **Cluster Pruning:** When analyzing the correlation matrix, do not just append new clusters. Actively cross-reference the hardcoded clusters in `STRATEGY.md` and propose **removing** any tickers or groups that have cooled off (correlation < 0.70) in the fresh matrix. This prevents permanent lockouts.
