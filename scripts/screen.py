...
class Screen:
    ...
    def parse_table(self, html):
        ...
        headers = [self.strip_tags(h) for h in re.findall(r"<th[^>]*>(.*?)</th>", html, re.S)]
        rows = []
        for row in re.findall(r"<tr[^>]*>(.*?)</tr>", html, re.S):
            cells = [self.strip_tags(c) for c in re.findall(r"<td[^>]*>(.*?)</td>", row, re.S)]
            if cells:
                rows.append(cells)
        return headers, rows
    ...
    def cmd_oversold(self, args):
        ...
        for cells in rows:
            ...
            picked.append((self.to_float(cells[rsi]) or 0.0, cells[sym], cells[name], p, extras))
        if not picked:
    ...
    def parse_quote(self, ticker):
        html_clean = re.sub(r"<!--.*?-->", "", self.fetch_symbol(ticker), flags=re.S)
    ...
    def cmd_quote(self, args):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = {executor.submit(fetch_wrapper, ticker): ticker for ticker in args.tickers}
    ...
    def cmd_forecast(self, args):
        ...
        def fetch_wrapper(ticker):
            try:
                ...
                dist = {k: self.last_int(pat, text) for k, pat in
    ...
    def cmd_news(self, args):
        ...
        def fetch_news(ticker):
            ...
            try:
                xml = self.fetch(f"https://news.google.com/rss/search?q={query}&hl=en-US&gl=US&ceid=US:en")
    ...
    def cmd_ipos(self, args):
        ...
        try:
            recent_headers, recent_rows = self.parse_table(self.fetch("/ipos/"))
        except:
            pass

        upcoming_headers, upcoming_rows = [], []
        try:
            upcoming_headers, upcoming_rows = self.parse_table(self.fetch("/ipos/calendar/"))
        except:
            pass
        ...
        def etoro_likely(row, headers, is_upcoming):
            ...
            if is_upcoming and ds_idx >= 0 and ds_idx < len(row):
                ds = self.to_float(row[ds_idx].replace('M','').replace('B',''))
            ...
            if is_upcoming and mc_idx >= 0 and mc_idx < len(row):
                mc = self.to_float(row[mc_idx].replace('M','').replace('B',''))
